...
public class PokerUtils {
  ...
  private static boolean isFlush(Suit[] suits) {
    ...
    for (Suit suit : suits) {
      if (initialSuit != suit) return false;
    }
    return true;
  }
  ...
  private static PokerHand checkFourOfKindOrFullHouse(Rank[] values) {
    ...
    for (int counter = MIN_INDEX; counter <= MAX_INDEX; counter++) {
      if (values[counter].getValue() == min) cardsWithMinRank++;
      if (values[counter].getValue() == max) cardsWithMaxRank++;
    }
  ...
  private static boolean isThreeOfKind(Rank[] values) {
    boolean a1, a2, a3;
    a1 =
        values[MIN_INDEX].getValue() == values[MIN_INDEX + 1].getValue()
  ...
  public static PokerHand findPokerHand(List<Card> cards) {
    ...
    if (cards.size() != SIZE_OF_HAND) throw new IllegalArgumentException("INVALID HAND");
    ...
    if (typeFound == PokerHand.NOTDETERMINED) typeFound = checkFourOfKindOrFullHouse(values);
